...
const PhotosTabAccommodation = ({ accommodation, photos, setPhotos }) => {
    ...
    useEffect(() => {
        if (accommodation && accommodation.photos) {
            fetchPhotos();
            setPhotos(accommodation.photos);
        }
    }, [accommodation]);
    ...
    const deleteSelectedPhotos = async () => {
        ...
        for (const photoId of selected) {
            try {
                await fetch(`${config.BACKEND_URL}/accommodations/${accommodation._id}/photos/${photoId}`, { method: 'DELETE' });
            } catch {}
        }
        await fetchPhotos();
        setSelectionMode(false);
...
export default PhotosTabAccommodation;
