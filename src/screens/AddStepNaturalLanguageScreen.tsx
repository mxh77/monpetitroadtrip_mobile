...
type Props = StackScreenProps<RootStackParamList, 'AddStepNaturalLanguage'>;

export default function AddStepNaturalLanguageScreen({ route, navigation }: Props) {
  const { roadtripId, refresh } = route.params;
  ...
  const handleCreateStep = async () => {
    if (!prompt.trim()) {
      Alert.alert('Erreur', 'Veuillez saisir une description pour votre étape.');
      return;
    }
    ...
    try {
      ...
      const apiUrl = `${config.BACKEND_URL}/roadtrips/${roadtripId}/steps/natural-language`;
      ...
      if (response.ok) {
        Alert.alert(
          'Succès',
          `Étape "${data.step.name}" créée avec succès !`,
          [
            {
              text: 'OK',
              onPress: () => {
                refresh(); // Actualiser la liste des étapes
                navigation.goBack();
              },
            },
          ]
  ...
  const examples = [
    "Visite du Louvre, Paris, demain à 10h et repartir à 16h",
    "Nuit à l'hôtel Ritz, Place Vendôme, Paris, arrivée ce soir 19h",
    "Arrêt station-service A6 Aire de Beaune dans 2 heures",
    "Randonnée au Mont-Blanc, Chamonix, départ 8h du refuge",
    "Pause déjeuner chez Paul, 74 rue de Rivoli, Paris, à 12h30"
  ];
    ...
    >
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Ajouter une étape via IA</Text>
        ...
        <Text style={styles.subtitle}>
          Décrivez votre étape en langage naturel et laissez l'IA faire le reste !
        </Text>

        <View style={styles.inputContainer}>
          <Text style={styles.label}>Description de votre étape :</Text>
          <TextInput
            style={styles.textInput}
            multiline
            numberOfLines={4}
            placeholder="Ex: Visite du château de Versailles demain à 10h..."
            value={prompt}
        ...
        <View style={styles.locationContainer}>
          ...
          <Text style={styles.locationDescription}>
            La géolocalisation est temporairement désactivée. L'IA utilisera uniquement les adresses spécifiées dans votre description.
          </Text>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.createButtonText}>Créer l'étape</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
